...
import (
	...
	"github.com/metaverse/truss/_example/skappservice-service/handlers"
...
func NewEndpoints() svc.Endpoints {
	// Business domain.
	var service pb.SkAppServiceServer
	{
	...
	// Endpoint domain.
	var (
		seckillEndpoint     = svc.MakeSeckillEndpoint(service)
		secinfoEndpoint     = svc.MakeSecInfoEndpoint(service)
		secinfolistEndpoint = svc.MakeSecInfoListEndpoint(service)
	)
...
func Run(cfg Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterSkAppServiceServer(s, srv)
